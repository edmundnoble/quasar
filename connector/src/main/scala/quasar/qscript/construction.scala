...
object construction {
  final case class Func[T[_[_]]]()(implicit corec: CorecursiveT[T]) {
    ...
    def IntLit[A](in: BigInt): FreeMapA[T, A] =
      Constant(EJson.int[T](in))
    def DecLit[A](in: BigDecimal): FreeMapA[T, A] =
      Constant(EJson.dec[T](in))
    def BoolLit[A](in: Boolean): FreeMapA[T, A] =
      Constant(EJson.bool[T](in))
    def StrLit[A](in: String): FreeMapA[T, A] =
      Constant(EJson.str[T](in))
    def NullLit[A]: FreeMapA[T, A] =
      Constant(EJson.nul[T])
    ...
    def ReduceIndex(i: Int \/ Int): FreeMapA[T, ReduceIndex] =
      Free.pure(qscript.ReduceIndex(i))
